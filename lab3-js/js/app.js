...
const iterationBox = document.querySelector('#iteration-box-js')
...
btn.addEventListener('click', () => {
    ...
    fermaPromise(inputValue)
        .then(n => ferma(n))
        .then(res => {
            const { error, result, log, iteration } = res
            if (error) {
                tableWrapper.innerHTML = ''
                answerBox.textContent = error
                iterationBox.textContent = 1
                return
            }
            ...
            iterationBox.textContent = iteration
            tableWrapper.innerHTML =
                `
